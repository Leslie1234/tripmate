  ...
  return (
    <section className="section">
      <div className="text-center mb-10">
        <h2 className="text-2xl md:text-3xl font-bold text-gray-900 mb-3">季节旅行灵感</h2>
        <p className="text-muted-foreground">不同的季节，不同的风景，找到属于你的最佳出行时间</p>
      </div>
        ...
        {seasons.map((s, i) => (
          <button
            ...
            className={cn(
              ...
              active === i ? s.activeColor : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
            )}
          >
            {s.emoji} {s.name}
          </button>
      ...
      <div className={`bg-gradient-to-br ${season.color} rounded-2xl p-6 md:p-8 transition-all duration-500`}>
        <div className="grid md:grid-cols-3 gap-6">
          {season.items.map((item) => (
            <div key={item.place} className="bg-white/70 backdrop-blur-sm rounded-xl p-5 hover:bg-white/90 transition-colors">
              <div className="text-sm font-medium text-muted-foreground mb-1">{item.place}</div>
              <h3 className="text-lg font-semibold text-gray-900 mb-2">{item.highlight}</h3>
